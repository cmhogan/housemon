...
fig = plt.figure(1,figsize=(6.40,7.20), dpi=100);
plt.clf()
...
for this_sid in sens_id:
    ...
    if this_sid == 9:
        ax3.plot(hr, data, label=locnames[plotnum])
    else:
        if np.max(data) > 10:
...
ax2.set(ylabel="temp (%sC)"%degree_sign)
ax3.set(xlabel="hour", ylabel="\u03BCg/m\u00b3")
#ax1.set_ylim(16,29)
